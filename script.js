document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- CITIES EXPLICITLY ALLOWED BY USER ---
    const ALLOWED_CITIES_FOR_DROPDOWN = [
        "Brecilien", // Note: Albion Online city names are typically capitalized.
        "Fort Sterling",
        "Lymhurst",
        "Bridgewatch",
        "Martlock",
    ...
    async function initializeSelectors() {
        ...
        if (rawLocationData) {
            // Mapear el array de objetos a un objeto { UniqueName: UniqueName }
            allLocations = rawLocationData.reduce((acc, loc) => {
                // Filter based on the ALLOWED_CITIES_FOR_DROPDOWN list
                // We use loc.UniqueName for comparison as that's what's in world.json
